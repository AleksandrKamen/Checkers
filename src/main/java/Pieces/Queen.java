        ...
        for (int i = 1; i <= 8; i++) {
            try {
                ...
                if (!board.isSquareEmpty(coordinates1)) {
                    ...
                    else if (!board.isSquareEmpty(new Coordinates(File.values()[coordinates1.file.ordinal() + 1], coordinates1.rank + 1)))
                        break;
                }
        ...
        for (int i = 1; i <= 8; i++) {
            try {
                ...
                if (!board.isSquareEmpty(coordinates2)) {
                    ...
                    else if (!board.isSquareEmpty(new Coordinates(File.values()[coordinates2.file.ordinal() - 1], coordinates2.rank - 1)))
                        break;
                }
                result.add(new CoordinatesShift(-i, -i));
            } catch (Exception e) {
